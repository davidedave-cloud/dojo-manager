...
export default function AdminPanel({ session, supabase }) {
  ...
  async function approveAthlete(id) {
    ...
    // Manda email di benvenuto automatica
    const athlete = athletes.find(a => a.id === id);
    if (athlete?.email && !athlete.email.includes("@dojo.local")) {
      try {
        await fetch("/api/welcome-email", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${import.meta.env.VITE_CRON_SECRET || "Karate123!"}` },
          body: JSON.stringify({ athlete: { ...athlete, status: "approved" } }),
        });
