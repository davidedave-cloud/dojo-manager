...
function PaymentsTab({ athletes, payments, supabase, onReload, setShowReceipt, BeltBadge }) {
  ...
  function generateReceipt(payment, athlete) {
    ...
    const stagione = anno >= 9 ? `${anno}/${anno+1}` : `${anno-1}/${anno}`;
    ...
    const causale = payment.payment_type === "annuale"
      ...
      : `Pagamento corso di ${a.course?.includes("Psico") ? "Psicomotricità" : "Karate"} anno ${stagione}`;
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .body { padding: 20px; }
  .row { display: flex; align-items: baseline; padding: 8px 0; border-bottom: 1px solid #ddd; gap: 12px; }
  .row-label { font-size: 10pt; color: #555; font-weight: bold; width: 140px; flex-shrink: 0; }
  .row-value { font-size: 10pt; color: #111; }
  .row-cf { font-size: 9pt; color: #777; margin-left: 8px; }
  ...
  .causale-row { display: flex; gap: 8px; padding: 8px 0; border-bottom: 1px solid #ddd; font-size: 10pt; }
...
<div class="page">
  <div class="header">
    <div class="logo-box">
      <img src="https://ccllvcdtehvbjroawomz.supabase.co/storage/v1/object/public/assets/Karate%20Do%20copia.png" alt="Logo" />
    </div>
    <div class="org-info">
    ...
    <div class="ricevuta-title">
      <h1>RICEVUTA</h1>
      <div class="meta">
        <span>DATA <strong>\${dataStr}</strong></span>
        <span>RICEVUTA N. <strong>\${receiptNum}</strong></span>
      </div>
    </div>
  </div>
  ...
  <div class="body">
    \${isMinore ? \`
    <div class="row">
      <span class="row-label">RICEVUTO DA:</span>
      <span class="row-value">\${a.parent_name || "—"}</span>
      <span class="row-cf">C.F. \${a.parent_cf || "—"}</span>
    </div>\` : \`
    <div class="row">
      <span class="row-label">RICEVUTO DA:</span>
      <span class="row-value">\${a.first_name} \${a.last_name}</span>
      <span class="row-cf">C.F. \${a.fiscal_code || "—"}</span>
    </div>\`}

    \${isMinore ? \`
    <div class="row">
      <span class="row-label">PER ATLETA:</span>
      <span class="row-value">\${a.first_name} \${a.last_name}</span>
      <span class="row-cf">C.F. \${a.fiscal_code || "—"}</span>
    </div>\` : ""}

    <div class="importo-box">
      <div class="importo-lettere">€ \${importoLettere}</div>
    </div>

    <div class="causale-row">
      <strong>CAUSALE:</strong>
      <span>\${causale}\${payment.receipt_note ? " — " + payment.receipt_note : ""}</span>
    </div>

    <div class="totale">
      <span class="totale-label">TOTALE</span>
      <span class="totale-value">€ \${importo.toFixed(2)}</span>
    </div>

    <div class="firma">
      <div class="firma-box">
        <div class="firma-line"></div>
        <div class="firma-label">IL RICEVENTE</div>
      </div>
    </div>
  </div>
</div>
