...
const TABS = ["Dashboard", "Atleti", "📋 Presenze", "Pagamenti", "Esami & Gare", "📢 Bacheca", "📊 Marketing"];
        ...
        {activeTab === "Dashboard" && (
          <div>
            ...
            {(() => {
              ...
              const expiring = athletes.filter(a => a.medical_expiry && new Date(a.medical_expiry) <= in30 && new Date(a.medical_expiry) >= today);
              const expired = athletes.filter(a => a.medical_expiry && new Date(a.medical_expiry) < today);
              return (
                <>
                  {expired.length > 0 && (
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setSelectedAthlete(null)}>
          <div style={{ background: "#1a1a0e", border: "1px solid #b8860b", borderRadius: 16, padding: 32, width: 520, maxWidth: "90vw", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              ...
              <button onClick={() => printModulo(selectedAthlete)} style={{ flex: 1, background: "rgba(234,179,8,0.15)", color: "#eab308", border: "1px solid #eab308", borderRadius: 8, padding: 10, cursor: "pointer", fontFamily: "inherit", fontSize: 13 }}>🖨️ Stampa Modulo</button>
            </div>
          </div>
        </div>
      )}
