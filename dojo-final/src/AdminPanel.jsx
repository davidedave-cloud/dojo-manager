...
const TABS = ["Dashboard", "Atleti", "📋 Presenze", "Pagamenti", "💸 Bilancio", "Esami & Gare", "📢 Bacheca", "📚 Risorse", "📊 Marketing"];
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          {todayLessons.map(l => (
            <div key={l.id} onClick={() => openLesson(l)} style={{ background: "#1a1a0e", border: "2px solid #b8860b", borderRadius: 12, padding: 18, marginBottom: 10, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div><div style={{ fontSize: 16, fontWeight: 700, color: "#daa520" }}>{l.lesson_type} — {l.location}</div><div style={{ fontSize: 12, color: "#888", marginTop: 3 }}>{l.lesson_time?.slice(0,5)} · {l.instructor}</div></div>
...
// COMPONENTE BILANCIO
// ============================================================
function BilancioTab({ payments, expenses, supabase, onReload }) {
  const now = new Date();
  const [selYear, setSelYear] = useState(now.getFullYear());
  const [selMonth, setSelMonth] = useState(now.getMonth() + 1);
  ...
  const monthExpenses = expenses.filter(e => e.period_month === selMonth && e.period_year === selYear);
  ...
  const yearExpenses = expenses.filter(e => e.period_year === selYear);
  ...
  const monthlyData = Array.from({ length: 12 }, (_, i) => {
    ...
    const exp = expenses.filter(e => e.period_month === m && e.period_year === selYear).reduce((s, e) => s + Number(e.amount), 0);
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 28 }}>
            <div style={{ background: "linear-gradient(135deg,#0d1a0d,#0a1a0a)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 12, padding: 20 }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: "#22c55e" }}>€{monthIncome}</div>
            ...
            <div style={{ background: "linear-gradient(135deg,#1a0d0d,#1a0a0a)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 12, padding: 20 }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: "#ef4444" }}>€{monthExpTotal}</div>
            ...
            <div style={{ background: monthBalance >= 0 ? "linear-gradient(135deg,#0d1a0d,#0a1a0a)" : "linear-gradient(135deg,#1a0d0d,#1a0a0a)", border: `1px solid ${monthBalance >= 0 ? "rgba(218,165,32,0.4)" : "rgba(239,68,68,0.4)"}`, borderRadius: 12, padding: 20 }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: monthBalance >= 0 ? "#daa520" : "#ef4444" }}>{monthBalance >= 0 ? "+" : ""}€{monthBalance}</div>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 28 }}>
            <div style={{ background: "linear-gradient(135deg,#0d1a0d,#0a1a0a)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 12, padding: 20 }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: "#22c55e" }}>€{yearIncome}</div>
            </div>
            <div style={{ background: "linear-gradient(135deg,#1a0d0d,#1a0a0a)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 12, padding: 20 }}>
              <div style={{ fontSize: 12, color: "#ef4444", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.08em" }}>❌ Uscite {selYear}</div>
              <div style={{ fontSize: 28, fontWeight: 700, color: "#ef4444" }}>€{yearExpTotal}</div>
            </div>
            <div style={{ background: yearBalance >= 0 ? "linear-gradient(135deg,#0d1a0d,#0a1a0a)" : "linear-gradient(135deg,#1a0d0d,#1a0a0a)", border: `1px solid ${yearBalance >= 0 ? "rgba(218,165,32,0.4)" : "rgba(239,68,68,0.4)"}`, borderRadius: 12, padding: 20 }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: yearBalance >= 0 ? "#daa520" : "#ef4444" }}>{yearBalance >= 0 ? "+" : ""}€{yearBalance}</div>
            </div>
          </div>
              ...
              <tbody>
                {monthlyData.map(m => (
                  <tr key={m.month} style={{ borderBottom: "1px solid #141408", opacity: m.income === 0 && m.expenses === 0 ? 0.4 : 1 }}>
                    ...
                    <td style={{ padding: "12px 20px", textAlign: "right", fontSize: 13, color: "#22c55e", fontWeight: 600 }}>€{m.income}</td>
                    <td style={{ padding: "12px 20px", textAlign: "right", fontSize: 13, color: m.expenses > 0 ? "#ef4444" : "#555" }}>€{m.expenses}</td>
                    <td style={{ padding: "12px 20px", textAlign: "right", fontSize: 14, fontWeight: 700, color: m.balance >= 0 ? "#daa520" : "#ef4444" }}>{m.balance >= 0 ? "+" : ""}€{m.balance}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ background: "#141408", borderTop: "2px solid #2a2a1a" }}>
                  ...
                  <td style={{ padding: "14px 20px", textAlign: "right", fontSize: 14, fontWeight: 700, color: "#22c55e" }}>€{yearIncome}</td>
                  <td style={{ padding: "14px 20px", textAlign: "right", fontSize: 14, fontWeight: 700, color: "#ef4444" }}>€{yearExpTotal}</td>
                  <td style={{ padding: "14px 20px", textAlign: "right", fontSize: 15, fontWeight: 700, color: yearBalance >= 0 ? "#daa520" : "#ef4444" }}>{yearBalance >= 0 ? "+" : ""}€{yearBalance}</td>
                </tr>
              </tfoot>
            </table>
          </div>
  ...
  return (
    <div style={{ fontFamily: "'Georgia','Times New Roman',serif", background: "#0d0d0d", minHeight: "100vh", color: "#e8e0d0" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "28px 24px" }}>
        ...
        {/* BILANCIO */}
        {activeTab === "💸 Bilancio" && (
          <BilancioTab payments={payments} expenses={expenses} supabase={supabase} onReload={() => loadData(true)} />
        )}
