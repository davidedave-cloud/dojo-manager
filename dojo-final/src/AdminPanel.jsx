...
export default function AdminPanel({ session, supabase }) {
  ...
  async function saveNewAthlete() {
    ...
    const payload = {
      ...
      belt: a.belt, course: a.course, location: a.location, discipline: a.discipline,
      is_minor: a.isMinor,
        ...
        {activeTab === "Pagamenti" && (
          <div>
            ...
            {athletes.filter(a => !a.parent_athlete_id && a.status === "approved").length === 0 ? (
              ...
              <div style={{ background: "#1a1a0e", border: "1px solid #2a2a1a", borderRadius: 12, overflow: "hidden" }}>
                <div style={{ padding: "16px 20px", borderBottom: "1px solid #2a2a1a", fontSize: 14, color: "#888" }}>Atleti approvati — {new Date().toLocaleDateString("it-IT", { month: "long", year: "numeric" })}</div>
                {athletes.filter(a => !a.parent_athlete_id && a.status === "approved").map(a => (
                  <div key={a.id} style={{ display: "flex", alignItems: "center", padding: "14px 20px", borderBottom: "1px solid #141408", gap: 16 }}>
                    <div style={{ flex: 1 }}><div style={{ fontWeight: 600, color: "#e8e0d0" }}>{a.first_name} {a.last_name}</div><div style={{ fontSize: 12, color: "#666" }}>{a.email}</div></div>
                    <BeltBadge belt={a.belt} />
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => markPaid(a.id)} style={{ background: "rgba(34,197,94,0.2)", color: "#22c55e", border: "1px solid #22c55e", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>+ Registra Pagamento</button>
                      <button onClick={() => setShowReceipt(a)} style={{ background: "rgba(74,158,255,0.15)", color: "#4a9eff", border: "1px solid #4a9eff", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>📄 Ricevuta</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setSelectedAthlete(null)}>
          <div style={{ background: "#1a1a0e", border: "1px solid #b8860b", borderRadius: 16, padding: 32, width: 520, maxWidth: "90vw", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            {[
              ...
              ["📍 Sede", selectedAthlete.location], ["🎯 Disciplina", selectedAthlete.discipline],
              ["🏥 Cert. medico", selectedAthlete.medical_expiry ? new Date(selectedAthlete.medical_expiry).toLocaleDateString("it-IT") : "—"],
          ...
          <div style={{ background: "#1a1a0e", border: "1px solid #b8860b", borderRadius: 16, padding: 32, width: 560, maxWidth: "100%", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 12 }}>
              ...
              <div>
                ...
                <select style={inputStyle} value={newAthlete.discipline} onChange={e => setF("discipline", e.target.value)}>
                  <option value="">Seleziona...</option>
