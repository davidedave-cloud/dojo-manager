...
const TABS = ["Dashboard", "Atleti", "📋 Presenze", "Pagamenti", "Esami & Gare", "📢 Bacheca", "📚 Risorse", "📊 Marketing"];
...
function PaymentsTab({ athletes, payments, supabase, onReload, setShowReceipt, BeltBadge }) {
  ...
  const unpaid = mainAthletes.filter(a => !paidIds.has(a.id) && !volunteerIds.has(a.id));
  const paid = mainAthletes.filter(a => paidIds.has(a.id));
  const totalMonth = monthPayments.reduce((s, p) => s + Number(p.amount), 0);

  // Annuale
  const annualPaid = payments.filter(p => p.payment_type === "annuale" && p.period_year === selYear && (p.status === "paid" || p.status === "external"));
  ...
  // Genera ricevuta PDF per atleta
  function generateReceipt(payment, athlete) {
    const a = athletes.find(x => x.id === payment.athlete_id) || athlete;
    if (!a) return;
    const html = `<!DOCTYPE html>
<html lang="it">
<head><meta charset="UTF-8"><title>Ricevuta</title>
<style>
  body { font-family: Georgia, serif; max-width: 600px; margin: 40px auto; color: #111; }
  .header { text-align: center; border-bottom: 2px solid #b8860b; padding-bottom: 20px; margin-bottom: 24px; }
  .logo { font-size: 28px; font-weight: bold; color: #b8860b; }
  .subtitle { font-size: 13px; color: #777; }
  .receipt-num { text-align: right; font-size: 12px; color: #999; margin-bottom: 20px; }
  .row { display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #eee; }
  .label { color: #666; font-size: 13px; }
  .value { font-size: 13px; font-weight: 600; }
  .total { display: flex; justify-content: space-between; background: #b8860b; color: white; padding: 16px 20px; border-radius: 8px; margin-top: 20px; }
...
<body>
<div class="header">
  <div class="logo">🥋 CINQUE CERCHI ASD</div>
  <div class="subtitle">Sezione Karate-do Tradizionale</div>
  <div class="subtitle">cinquecerchikaratedo@gmail.com</div>
</div>
<div class="receipt-num">
  <strong>RICEVUTA N°</strong> ${payment.receipt_number || payment.id?.slice(0,8).toUpperCase()}<br>
  Data: ${new Date(payment.paid_at || Date.now()).toLocaleDateString("it-IT")}
</div>
<div class="row"><span class="label">Intestatario</span><span class="value">${a.first_name} ${a.last_name}</span></div>
<div class="row"><span class="label">Tipo pagamento</span><span class="value">${payment.payment_type === "annuale" ? "Tessera annuale" : "Quota mensile"}</span></div>
<div class="row"><span class="label">Periodo</span><span class="value">${payment.payment_type === "annuale" ? `Anno ${payment.period_year}` : `${["Gennaio","Febbraio","Marzo","Aprile","Maggio","Giugno","Luglio","Agosto","Settembre","Ottobre","Novembre","Dicembre"][(payment.period_month||1)-1]} ${payment.period_year}`}</span></div>
<div class="row"><span class="label">Corso</span><span class="value">${a.course} · ${a.location}</span></div>
${payment.receipt_note ? `<div class="row"><span class="label">Note</span><span class="value">${payment.receipt_note}</span></div>` : ""}
<div class="total">
  <span class="total-label">TOTALE PAGATO</span>
  <span class="total-value">€${Number(payment.amount).toFixed(2)}</span>
</div>
<div class="footer">
  Cinque Cerchi ASD — C.F./P.IVA: ____________<br>
...
// COMPONENTE PRESENZE
// ============================================================
function PresenzeTab({ athletes, lessons, supabase, onReload, BeltBadge }) {
  const [view, setView] = useState("oggi"); // "oggi" | "storico"
  ...
  const [lessonForm, setLessonForm] = useState({
    ...
    time: "18:00", type: "Karate", instructor: "", location: "Argenta",
  });
  ...
  async function savePresenze() {
    setSaving(true);
    await supabase.from("attendances").delete().eq("lesson_id", selectedLesson.id);
    const toInsert = Object.entries(presenze)
      .filter(([, present]) => present)
      .map(([athlete_id]) => ({ lesson_id: selectedLesson.id, athlete_id, present: true }));
    if (toInsert.length > 0) await supabase.from("attendances").insert(toInsert);
    await onReload();
  ...
  if (selectedLesson) return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h2 style={{ color: "#daa520", fontSize: 22, margin: 0 }}>{selectedLesson.lesson_type}</h2>
          <div style={{ fontSize: 13, color: "#888", marginTop: 4 }}>{new Date(selectedLesson.lesson_date).toLocaleDateString("it-IT", { weekday: "long", day: "numeric", month: "long" })} · {selectedLesson.lesson_time?.slice(0,5)} · {selectedLesson.location}</div>
        </div>
    ...
    <div>
      ...
      {todayLessons.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          ...
          {todayLessons.map(l => (
            <div key={l.id} onClick={() => openLesson(l)} style={{ background: "#1a1a0e", border: "2px solid #b8860b", borderRadius: 12, padding: 18, marginBottom: 10, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div><div style={{ fontSize: 16, fontWeight: 700, color: "#daa520" }}>{l.lesson_type}</div><div style={{ fontSize: 12, color: "#888", marginTop: 3 }}>{l.lesson_time?.slice(0,5)} · {l.location} · {l.instructor}</div></div>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontSize: 13, color: "#22c55e" }}>{l.attendances?.length || 0} presenti</span>
                <span style={{ color: "#daa520", fontSize: 18 }}>→</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        {pastLessons.slice(0, 20).map(l => (
          <div key={l.id} onClick={() => openLesson(l)} style={{ background: "#1a1a0e", border: "1px solid #2a2a1a", borderRadius: 12, padding: 16, marginBottom: 8, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}
            onMouseEnter={e => e.currentTarget.style.borderColor = "#b8860b"}
            onMouseLeave={e => e.currentTarget.style.borderColor = "#2a2a1a"}>
            <div>
      ...
      {showNewLesson && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#1a1a0e", border: "1px solid #b8860b", borderRadius: 16, padding: 32, width: 440, maxWidth: "90vw" }}>
            ...
            <div style={{ marginBottom: 12 }}><label style={{ display: "block", fontSize: 11, color: "#777", marginBottom: 4 }}>TIPO LEZIONE</label><input style={{ ...iStyle2, width: "100%", boxSizing: "border-box" }} value={lessonForm.type} onChange={e => setLessonForm(p => ({ ...p, type: e.target.value }))} placeholder="Es: Karate, Psicomotricità..." /></div>
            <div style={{ marginBottom: 12 }}><label style={{ display: "block", fontSize: 11, color: "#777", marginBottom: 4 }}>ISTRUTTORE</label><input style={{ ...iStyle2, width: "100%", boxSizing: "border-box" }} value={lessonForm.instructor} onChange={e => setLessonForm(p => ({ ...p, instructor: e.target.value }))} placeholder="Nome istruttore" /></div>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", fontSize: 11, color: "#777", marginBottom: 4 }}>SEDE</label>
              <div style={{ display: "flex", gap: 8 }}>
                {["Argenta", "S.M. Codifiume"].map(loc => (
                  <button key={loc} onClick={() => setLessonForm(p => ({ ...p, location: loc }))} style={{ flex: 1, background: lessonForm.location === loc ? "linear-gradient(135deg,#b8860b,#daa520)" : "#141408", color: lessonForm.location === loc ? "#0d0d0d" : "#888", border: `1px solid ${lessonForm.location === loc ? "#daa520" : "#2a2a1a"}`, borderRadius: 8, padding: "10px", cursor: "pointer", fontSize: 13, fontFamily: "inherit", fontWeight: lessonForm.location === loc ? 700 : 400 }}>{loc}</button>
                ))}
              </div>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
...
// ============================================================
function BachecaTab({ news, supabase, onReload, athletes }) {
...
// COMPONENTE RISORSE
// ============================================================
function RisorseTab({ resources, supabase, onReload }) {
  const [showForm, setShowForm] = useState(false);
...
// ============================================================
function EsamiTab({ athletes, exams, supabase, onReload, BeltBadge, BELT_ORDER }) {
  const [showNewExam, setShowNewExam] = useState(false);
  const [showCandidates, setShowCandidates] = useState(null);
  const [examForm, setExamForm] = useState({ date: "", location: "Argenta", notes: "" });
  ...
  async function createExam() {
    ...
    setSaving(true);
    const { data: exam } = await supabase.from("events").insert({
      event_date: examForm.date,
      event_type: "Esame Cinture",
      location: examForm.location,
      notes: examForm.notes,
      is_public: true,
    }).select().single();

    if (exam && selectedCandidates.length > 0) {
    ...
    setExamForm({ date: "", location: "Argenta", notes: "" });
  ...
  async function saveResults(examId) {
    ...
    for (const ep of exam.event_participants || []) {
      ...
      // Aggiorna cintura atleta se promosso
  ...
  if (activeExam) {
    ...
    return (
      <div>
        <button onClick={() => { setActiveExam(null); setResults({}); }} style={{ background: "none", border: "none", color: "#daa520", cursor: "pointer", fontSize: 13, fontFamily: "inherit", marginBottom: 16 }}>← Torna agli esami</button>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
          <div>
            <h2 style={{ color: "#daa520", fontSize: 22, margin: 0 }}>📝 Inserisci Risultati</h2>
            <div style={{ fontSize: 13, color: "#888", marginTop: 4 }}>{new Date(exam.event_date).toLocaleDateString("it-IT", { weekday: "long", day: "numeric", month: "long", year: "numeric" })} · {exam.location}</div>
          </div>
        </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6 }}>
                    ...
                    <BeltBadge belt={res.newBelt || getNextBelt(a.belt)} />
                  </div>
                </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <h2 style={{ color: "#daa520", fontSize: 22 }}>🏆 Esami Cinture</h2>
        <button onClick={() => setShowNewExam(true)} style={{ background: "linear-gradient(135deg,#b8860b,#daa520)", color: "#0d0d0d", border: "none", borderRadius: 8, padding: "10px 20px", cursor: "pointer", fontWeight: 700, fontSize: 13, fontFamily: "inherit" }}>+ Nuovo Esame</button>
      </div>

      {exams.length === 0 ? (
        <div style={{ textAlign: "center", padding: 60, color: "#555" }}><div style={{ fontSize: 36, marginBottom: 12 }}>🏆</div><div>Nessun esame programmato. Clicca "+ Nuovo Esame" per iniziare!</div></div>
      ) : exams.map(e => {
        ...
        return (
          <div key={e.id} style={{ background: "#1a1a0e", border: "1px solid #2a2a1a", borderRadius: 12, padding: 24, marginBottom: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 18, fontWeight: 700, color: "#daa520" }}>🥋 Esame Cinture</div>
                <div style={{ fontSize: 13, color: "#888", marginTop: 4 }}>{new Date(e.event_date).toLocaleDateString("it-IT", { weekday: "long", day: "numeric", month: "long", year: "numeric" })} · {e.location}</div>
                {e.notes && <div style={{ fontSize: 12, color: "#666", marginTop: 4 }}>{e.notes}</div>}
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                {!hasResults && <button onClick={() => setActiveExam(e.id)} style={{ background: "rgba(218,165,32,0.15)", color: "#daa520", border: "1px solid #daa520", borderRadius: 8, padding: "8px 16px", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>📝 Inserisci Risultati</button>}
                {hasResults && <button onClick={() => setActiveExam(e.id)} style={{ background: "rgba(74,158,255,0.15)", color: "#4a9eff", border: "1px solid #4a9eff", borderRadius: 8, padding: "8px 16px", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>✏️ Modifica</button>}
              </div>
            </div>

            {hasResults && (
              <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                <span style={{ background: "rgba(34,197,94,0.15)", color: "#22c55e", border: "1px solid #22c55e", borderRadius: 99, padding: "4px 14px", fontSize: 12, fontWeight: 700 }}>✓ {promossi} promossi</span>
                {rimandati > 0 && <span style={{ background: "rgba(239,68,68,0.15)", color: "#ef4444", border: "1px solid #ef4444", borderRadius: 99, padding: "4px 14px", fontSize: 12, fontWeight: 700 }}>✗ {rimandati} rimandati</span>}
              </div>
            )}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {e.event_participants?.map(ep => {
                ...
                if (!a) return null;
                return (
                  <div key={ep.athlete_id} style={{ background: "#141408", borderRadius: 10, padding: "10px 14px", border: `1px solid ${ep.status === "promosso" ? "rgba(34,197,94,0.3)" : ep.status === "rimandato" ? "rgba(239,68,68,0.3)" : "#2a2a1a"}` }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#e8e0d0" }}>{a.first_name} {a.last_name}</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
                      <BeltBadge belt={a.belt} />
                      {ep.status === "promosso" && ep.new_belt && <><span style={{ color: "#22c55e", fontSize: 11 }}>→</span><BeltBadge belt={ep.new_belt} /></>}
                      {ep.status === "rimandato" && <span style={{ fontSize: 11, color: "#ef4444" }}>✗ rimandato</span>}
                    </div>
                  </div>
                );
              })}
      ...
      {showNewExam && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
          <div style={{ background: "#1a1a0e", border: "1px solid #b8860b", borderRadius: 16, padding: 32, width: 520, maxWidth: "100%", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              <h3 style={{ color: "#daa520", margin: 0 }}>Nuovo Esame Cinture</h3>
              <button onClick={() => setShowNewExam(false)} style={{ background: "none", border: "none", color: "#666", fontSize: 20, cursor: "pointer" }}>✕</button>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
              <div><label style={{ display: "block", fontSize: 11, color: "#777", marginBottom: 4 }}>DATA *</label><input type="date" style={iS} value={examForm.date} onChange={e => setExamForm(p => ({ ...p, date: e.target.value }))} /></div>
              <div>
                <label style={{ display: "block", fontSize: 11, color: "#777", marginBottom: 4 }}>SEDE</label>
                <select style={iS} value={examForm.location} onChange={e => setExamForm(p => ({ ...p, location: e.target.value }))}>
                  <option>Argenta</option><option>S.M. Codifiume</option>
                </select>
              </div>
            </div>
            <div style={{ marginBottom: 20 }}><label style={{ display: "block", fontSize: 11, color: "#777", marginBottom: 4 }}>NOTE</label><input style={iS} value={examForm.notes} onChange={e => setExamForm(p => ({ ...p, notes: e.target.value }))} placeholder="Es: Esame invernale 2026..." /></div>

            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, color: "#777", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.08em" }}>Seleziona Candidati ({selectedCandidates.length} selezionati)</div>
                ...
                {approvedAthletes.map(a => {
                  ...
                  return (
                    <div key={a.id} onClick={() => setSelectedCandidates(prev => selected ? prev.filter(id => id !== a.id) : [...prev, a.id])}
                      style={{ background: selected ? "rgba(34,197,94,0.1)" : "#141408", border: `1px solid ${selected ? "#22c55e" : "#2a2a1a"}`, borderRadius: 8, padding: "10px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 24, height: 24, borderRadius: "50%", background: selected ? "#22c55e" : "#2a2a1a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, flexShrink: 0 }}>{selected ? "✓" : ""}</div>
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 600, color: selected ? "#22c55e" : "#e8e0d0" }}>{a.first_name} {a.last_name}</div>
                        <div style={{ marginTop: 2 }}><BeltBadge belt={a.belt} /></div>
                      </div>
                    </div>
                  );
                })}
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={createExam} disabled={saving || !examForm.date} style={{ flex: 1, background: "linear-gradient(135deg,#b8860b,#daa520)", color: "#0d0d0d", border: "none", borderRadius: 10, padding: 14, cursor: "pointer", fontWeight: 700, fontSize: 14, fontFamily: "inherit", opacity: saving || !examForm.date ? 0.6 : 1 }}>
                {saving ? "Creando..." : "✓ Crea Esame"}
              </button>
              <button onClick={() => setShowNewExam(false)} style={{ background: "#2a2a1a", color: "#888", border: "1px solid #3a3a2a", borderRadius: 10, padding: "14px 18px", cursor: "pointer", fontFamily: "inherit" }}>Annulla</button>
            </div>
          </div>
        </div>
      )}
...
export default function AdminPanel({ session, supabase }) {
  ...
  async function loadData() {
    setLoading(true);
    const [a, p, l, e, n, r] = await Promise.all([
