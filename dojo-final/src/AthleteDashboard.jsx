  ...
  <div>
    ...
    {payments.filter(p => p.status === "paid").length === 0
      ...
      : payments.filter(p => p.status === "paid").map(p => {
        ...
        const a = p.athletes || athlete;
        const intestatario = a.is_minor ? (a.parent_name || athlete.first_name + " " + athlete.last_name) : (a.first_name + " " + a.last_name);
        const cf = a.is_minor ? (a.parent_cf || athlete.fiscal_code) : (a.fiscal_code || athlete.fiscal_code);
        return (
          <div key={p.id} style={{ background: "#131008", border: "1px solid #2a2010", borderRadius: 12, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, gap: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "#5a5040", marginTop: 2 }}>
                {a.first_name} {a.last_name} · {p.receipt_number}
              </div>
            </div>
            <span style={{ fontSize: 15, fontWeight: 700, color: "#daa520" }}>€{p.amount}</span>
            <button onClick={() => {
              ...
              win.document.write("<!DOCTYPE html><html lang='it'><head><meta charset='UTF-8'><title>Ricevuta "+receiptNum+"</title><style>@page{size:A4;margin:20mm}*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:11pt;color:#111}.page{max-width:700px;margin:0 auto;border:1px solid #ccc}.header{display:flex;align-items:center;padding:16px 20px;border-bottom:2px solid #1a1408;gap:16px}.logo-box{width:80px;height:80px}.logo-box img{width:100%;height:100%;object-fit:contain}.org-info{font-size:9pt;color:#444;line-height:1.6}.org-name{font-weight:bold;font-size:10pt}.ricevuta-title{margin-left:auto;text-align:right}.ricevuta-title h1{font-size:28pt;font-weight:900;letter-spacing:2px}.meta{display:flex;gap:30px;justify-content:flex-end;font-size:10pt;margin-top:4px}.body{padding:20px}table{width:100%;border-collapse:collapse}td{padding:8px 6px;border-bottom:1px solid #ddd;font-size:10pt}.label{color:#555;font-weight:bold;width:140px}.importo-box{background:#f5f5f5;border:1px solid #ddd;border-radius:4px;padding:12px 20px;margin:16px 0;text-align:center}.importo-lettere{font-size:14pt;font-style:italic;font-weight:bold}.totale{display:flex;justify-content:space-between;background:#1a1408;color:white;padding:14px 20px;margin-top:16px}.totale-label{font-size:13pt;font-weight:bold}.totale-value{font-size:16pt;font-weight:bold}.firma{display:flex;justify-content:flex-end;margin-top:30px;padding-right:20px}.firma-line{border-top:1px solid #333;width:200px;margin-top:40px}.firma-label{font-size:9pt;color:#555;margin-top:4px;text-align:center}</style></head><body><div class='page'><div class='header'><div class='logo-box'><img src='"+logoUrl+"'/></div><div class='org-info'><div class='org-name'>Associazione Sportiva Dilettantistica</div><div class='org-name'>CINQUE CERCHI</div><div>Sezione Karate-do Tradizionale</div><div>cinquecerchikaratedo@gmail.com</div></div><div class='ricevuta-title'><h1>RICEVUTA</h1><div class='meta'><span>DATA <strong>"+dataStr+"</strong></span><span>N. <strong>"+receiptNum+"</strong></span></div></div></div><div class='body'><table><tr><td class='label'>RICEVUTO DA:</td><td>"+intestatario+" <span style='font-size:9pt;color:#777'>C.F. "+cf+"</span></td></tr>"+(a.is_minor ? "<tr><td class='label'>PER ATLETA:</td><td>"+a.first_name+" "+a.last_name+"</td></tr>" : "")+"<tr><td class='label'>IMPORTO:</td><td><div class='importo-box'><span class='importo-lettere'>€ "+Number(p.amount).toFixed(2)+"</span></div></td></tr><tr><td class='label'>CAUSALE:</td><td>"+causale+"</td></tr></table><div class='totale'><span class='totale-label'>TOTALE</span><span class='totale-value'>€ "+Number(p.amount).toFixed(2)+"</span></div><div class='firma'><div><div class='firma-line'></div><div class='firma-label'>IL RICEVENTE</div></div></div></div></div><script>window.onload=()=>window.print();</script></body></html>");
              win.document.close();
