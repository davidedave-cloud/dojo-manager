...
import {
  BELT_ORDER, COURSES, LOCATIONS, HOW_FOUND, STEP_LABELS, TOTAL_STEPS,
  emptyReg, inputStyle, labelStyle, BeltBadge, BeltProgress, StepIndicator, SectionTitle
} from "./athleteConstants.jsx";
...
export default function AthletePortal({ session, supabase }) {
  ...
  const [settingsForm, setSettingsForm] = useState({ newEmail: "", newPassword: "", confirmPassword: "", currentPassword: "" });
    ...
    <div style={{ fontFamily: "'Palatino Linotype','Book Antiqua',Palatino,serif", background: "#0a0905", minHeight: "100vh", color: "#e8e0d0", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "linear-gradient(135deg,#131008,#0d0c07)", border: "1px solid #b8860b", borderRadius: 20, padding: "48px 40px", width: 420, maxWidth: "90vw", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 13, color: "#5a5040", marginTop: 8, lineHeight: 1.6 }}>Per la tua sicurezza, scegli una password personale prima di accedere.</div>
        </div>
        <div style={{ marginBottom: 14 }}><label style={labelStyle}>Nuova password *</label><input type="password" style={inputStyle} value={settingsForm.newPassword} onChange={e => setSettingsForm(p => ({...p, newPassword: e.target.value}))} placeholder="Minimo 8 caratteri" /></div>
        <div style={{ marginBottom: 20 }}><label style={labelStyle}>Conferma password *</label><input type="password" style={inputStyle} value={settingsForm.confirmPassword} onChange={e => setSettingsForm(p => ({...p, confirmPassword: e.target.value}))} placeholder="Ripeti la password" /></div>
        {settingsError && <div style={{ marginBottom: 14, padding: "10px 14px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, fontSize: 12, color: "#ef4444" }}>⚠️ {settingsError}</div>}
        <button disabled={settingsSaving} onClick={async () => {
          if (!settingsForm.newPassword || settingsForm.newPassword.length < 8) { setSettingsError("La password deve avere almeno 8 caratteri."); return; }
          if (settingsForm.newPassword !== settingsForm.confirmPassword) { setSettingsError("Le password non coincidono."); return; }
          setSettingsSaving(true); setSettingsError("");
          const { error } = await supabase.auth.updateUser({ password: settingsForm.newPassword });
          if (error) { setSettingsError("Errore: " + error.message); setSettingsSaving(false); return; }
          setMustChangePassword(false);
          setSettingsForm(p => ({...p, newPassword: "", confirmPassword: ""}));
          setSettingsSaving(false);
        }} style={{ width: "100%", background: "linear-gradient(135deg,#b8860b,#daa520)", color: "#0a0905", border: "none", borderRadius: 10, padding: "14px", cursor: "pointer", fontSize: 15, fontWeight: 700, fontFamily: "inherit", opacity: settingsSaving ? 0.7 : 1 }}>
          {settingsSaving ? "Salvataggio..." : "✓ Salva e Accedi"}
        </button>
      </div>
    </div>
  );
                  ...
                  <div key={i} style={{ background: "#131008", border: `1px solid ${color}33`, borderRadius: 14, padding: 20, marginBottom: 14 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div>
                        ...
                        {ep.events?.notes && <div style={{ fontSize: 12, color: "#5a5040", marginTop: 2 }}>📝 {ep.events.notes}</div>}
                      </div>
            ...
            <div>
              ...
              <div style={{ background: "#131008", border: "1px solid #2a2010", borderRadius: 14, padding: 24, marginBottom: 16 }}>
                ...
                <button disabled={settingsSaving} onClick={async () => {
                  ...
                  if (!error) {
                    await supabase.from("athletes").update({ email: settingsForm.newEmail }).eq("id", athlete.id);
                    setSettingsSuccess("email"); setSettingsForm(p => ({...p, newEmail: ""}));
                  } else { setSettingsError("email"); setSettingsSuccess(error.message); }
                  setSettingsSaving(false);
                }} style={{ background: "linear-gradient(135deg,#b8860b,#daa520)", color: "#0a0905", border: "none", borderRadius: 8, padding: "10px 24px", cursor: "pointer", fontWeight: 700, fontSize: 13, fontFamily: "inherit" }}>
                  Aggiorna Email
                </button>
              </div>
