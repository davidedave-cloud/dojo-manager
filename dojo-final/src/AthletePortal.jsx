...
function AddMemberModal({ onClose, onSave, parentAthlete }) {
...
export default function AthletePortal({ session, supabase }) {
  ...
  const [alreadyMember, setAlreadyMember] = useState({ email: "", password: "", confirm: "", error: "", success: false, loading: false });
  ...
  async function loadAthleteData(userId) {
    ...
    if (data) {
      setAthlete(data);
      setScreen("dashboard");
      const [p, n, e, fam] = await Promise.all([
      ...
      setResources((await supabase.from("resources").select("*").order("created_at", { ascending: false })).data || []);
    }
  }
  ...
  async function handleAddMember(memberData) {
    const { error } = await supabase.from("athletes").insert({
      ...
      email: athlete.email, mobile: athlete.mobile,
  ...
  async function handleAlreadyMember() {
    const { email, password, confirm } = alreadyMember;
    if (!email || !password) { setAlreadyMember(p => ({...p, error: "Inserisci email e password."})); return; }
    if (password !== confirm) { setAlreadyMember(p => ({...p, error: "Le password non coincidono."})); return; }
    if (password.length < 6) { setAlreadyMember(p => ({...p, error: "La password deve avere almeno 6 caratteri."})); return; }
    setAlreadyMember(p => ({...p, loading: true, error: ""}));
    // Check if athlete exists in DB with this email
    const { data: existing } = await supabase.from("athletes").select("id, first_name, last_name, email").eq("email", email).single();
    if (!existing) {
      setAlreadyMember(p => ({...p, loading: false, error: "Email non trovata nel sistema. Contatta la segreteria oppure usa il modulo di iscrizione."}));
      return;
    }
    if (existing.user_id) {
      setAlreadyMember(p => ({...p, loading: false, error: "Questo account esiste già. Usa il login normale."}));
      return;
    }
    // Create auth account
    const { data: authData, error: authError } = await supabase.auth.signUp({ email, password });
    if (authError) { setAlreadyMember(p => ({...p, loading: false, error: "Errore: " + authError.message})); return; }
    // Link user_id to existing athlete record
  ...
  async function handleLogout() {
    ...
    setAthlete(null); setFamilyMembers([]); setScreen("home"); setActiveTab("profilo");
  }
  ...
  if (screen === "home") return (
    <div style={{ ...BASE, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ textAlign: "center", zIndex: 1, opacity: mounted ? 1 : 0, transform: mounted ? "translateY(0)" : "translateY(20px)", transition: "all 0.8s ease" }}>
        <img src="https://ccllvcdtehvbjroawomz.supabase.co/storage/v1/object/public/assets/Karate%20Do%20w.png" alt="Cinque Cerchi ASD" style={{ width: 200, height: 200, objectFit: "contain", marginBottom: 8, mixBlendMode: "lighten" }} />
  ...
  if (screen === "already-member") return (
    <div style={{ ...BASE, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "linear-gradient(135deg,#131008,#0d0c07)", border: "1px solid #2a2010", borderRadius: 20, padding: "48px 40px", width: 420, maxWidth: "90vw", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
        {!alreadyMember.success ? (
          <>
            ...
            <div style={{ marginBottom: 14 }}><label style={labelStyle}>La tua email *</label><input type="email" style={inputStyle} value={alreadyMember.email} onChange={e => setAlreadyMember(p => ({...p, email: e.target.value}))} placeholder="la-tua@email.com" /></div>
            <div style={{ marginBottom: 14 }}><label style={labelStyle}>Scegli una password *</label><input type="password" style={inputStyle} value={alreadyMember.password} onChange={e => setAlreadyMember(p => ({...p, password: e.target.value}))} placeholder="Minimo 6 caratteri" /></div>
            <div style={{ marginBottom: 20 }}><label style={labelStyle}>Conferma password *</label><input type="password" style={inputStyle} value={alreadyMember.confirm} onChange={e => setAlreadyMember(p => ({...p, confirm: e.target.value}))} placeholder="Ripeti la password" /></div>
            ...
            <button onClick={handleAlreadyMember} disabled={alreadyMember.loading} style={{ width: "100%", background: "linear-gradient(135deg,#b8860b,#daa520)", color: "#0a0905", border: "none", borderRadius: 10, padding: "14px", cursor: "pointer", fontSize: 15, fontWeight: 700, fontFamily: "inherit", opacity: alreadyMember.loading ? 0.7 : 1 }}>
              {alreadyMember.loading ? "Verifica in corso..." : "✓ Crea il mio accesso"}
            </button>
            <button onClick={() => setScreen("home")} style={{ display: "block", margin: "16px auto 0", background: "none", border: "none", color: "#4a3a2a", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>← Home</button>
          </>
        ) : (
        ...
        {!regSuccess ? (<>
          ...
          {regStep === 2 && (
            <div>
              ...
              <div style={{ marginBottom: 18 }}>
                <label style={labelStyle}>Cintura attuale</label>
                <select style={inputStyle} value={reg.belt} onChange={e => setField("belt", e.target.value)}>
          ...
          {regStep === 4 && (
            <div>
              ...
              <div style={{ border: "2px dashed #2a2010", borderRadius: 10, padding: "24px", textAlign: "center", cursor: "pointer", background: reg.medicalFile ? "rgba(34,197,94,0.05)" : "#0a0905" }} onClick={() => document.getElementById("medFile").click()}>
                {reg.medicalFile
                  ? <div><div style={{ fontSize: 24, marginBottom: 6 }}>✅</div><div style={{ fontSize: 12, color: "#22c55e" }}>{reg.medicalFile.name}</div></div>
                  : <div><div style={{ fontSize: 28, marginBottom: 6 }}>📎</div><div style={{ fontSize: 12, color: "#8a7a6a" }}>Clicca per caricare (facoltativo)</div><div style={{ fontSize: 11, color: "#5a5040", marginTop: 4 }}>JPG, PNG o PDF — max 5MB</div></div>}
              ...
              <div style={{ fontSize: 11, color: "#5a5040", marginTop: 8 }}>Se non hai il certificato ora, puoi portarlo alla prima lezione.</div>
            </div>
          )}
  ...
  if (screen === "dashboard" && athlete) {
    const TABS = [
      ...
      { key: "esami", label: "Esami & Gare", icon: "🏆" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "28px" }}>
          {activeTab === "profilo" && (
            <div style={{ background: "linear-gradient(135deg,#131008,#1a1408)", border: "1px solid #2a2010", borderRadius: 16, padding: 26 }}>
              ...
              <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #2a2010", display: "flex", gap: 10, flexWrap: "wrap" }}>
                <button onClick={() => printModuloTesseramento(athlete, "2025/2026")} style={{ background: "rgba(218,165,32,0.1)", color: "#daa520", border: "1px solid rgba(218,165,32,0.3)", borderRadius: 8, padding: "10px 20px", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>
                  🖨️ Stampa Modulo di Tesseramento
                </button>
                <button onClick={() => { setProfileForm({ email: athlete.email, mobile: athlete.mobile, phone: athlete.phone, address: athlete.address, city: athlete.city, zip: athlete.zip }); setEditProfile(true); }} style={{ background: "rgba(74,158,255,0.1)", color: "#4a9eff", border: "1px solid rgba(74,158,255,0.3)", borderRadius: 8, padding: "10px 20px", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>
                  ✏️ Modifica dati
                </button>
              </div>
          ...
          {activeTab === "pagamenti" && (
            <div>
              ...
              {payments.length === 0 ? <div style={{ textAlign: "center", padding: 60, color: "#5a5040" }}>Nessun pagamento registrato.</div>
                : payments.map(p => (
                  <div key={p.id} style={{ background: "#131008", border: "1px solid #2a2010", borderRadius: 12, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "#e8e0d0" }}>{p.payment_type === "annuale" ? `🎫 Tessera ${p.period_year}` : `📅 ${p.period_month}/${p.period_year}`}</div>
                      <div style={{ fontSize: 11, color: "#5a5040", marginTop: 2 }}>{p.receipt_number}</div>
                    </div>
          ...
          {activeTab === "esami" && (
            <div>
              ...
              {exams.length === 0 ? <div style={{ textAlign: "center", padding: 60, color: "#5a5040" }}>Nessun evento in programma.</div>
                : exams.map((ep, i) => (
                  <div key={i} style={{ background: "#131008", border: "1px solid #2a2010", borderRadius: 14, padding: 20, marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div><div style={{ fontSize: 17, fontWeight: 700, color: "#daa520" }}>{ep.events?.event_type}</div><div style={{ fontSize: 12, color: "#5a5040", marginTop: 4 }}>{ep.events?.event_date ? new Date(ep.events.event_date).toLocaleDateString("it-IT") : ""}</div></div>
                    <span style={{ background: "rgba(74,158,255,0.15)", color: "#4a9eff", border: "1px solid rgba(74,158,255,0.3)", borderRadius: 99, padding: "4px 14px", fontSize: 11, fontWeight: 700 }}>{ep.status}</span>
                  </div>
                ))}
            </div>
          )}
          ...
          {activeTab === "risorse" && (
            <div>
              ...
              <div style={{ fontSize: 13, color: "#5a5040", marginBottom: 24, lineHeight: 1.7 }}>Documenti, dispense e materiali di studio messi a disposizione dal Sensei.</div>
              {resources.length === 0 ? (
                <div style={{ textAlign: "center", padding: "60px 20px", color: "#5a5040", background: "#0d0c07", borderRadius: 14, border: "1px dashed #2a2010" }}>
                  <div style={{ fontSize: 36, marginBottom: 12 }}>📄</div>
                  <div style={{ fontSize: 14, marginBottom: 8 }}>Nessun documento disponibile</div>
                  <div style={{ fontSize: 12, color: "#3a3020" }}>Il Sensei caricherà presto materiali di studio</div>
                </div>
              ) : resources.map(r => (
                <a key={r.id} href={r.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 16, background: "#131008", border: "1px solid #2a2010", borderRadius: 12, padding: "16px 20px", marginBottom: 12, textDecoration: "none", cursor: "pointer" }}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#3a3020", marginTop: 4 }}>{new Date(r.created_at).toLocaleDateString("it-IT")}</div>
                  </div>
                  <div style={{ fontSize: 12, color: "#b8860b", flexShrink: 0 }}>Scarica →</div>
                </a>
              ))}
            </div>
          )}
