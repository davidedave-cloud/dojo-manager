...
export default function AthletePortal({ session, supabase }) {
  ...
  async function handleAlreadyMember() {
    ...
    // Check if athlete exists in DB with this email (case-insensitive)
    ...
    if (!existing) {
      setAlreadyMember(p => ({...p, loading: false, error: "Email non trovata nel sistema. Controlla di aver scritto bene l'email oppure contatta la segreteria."}));
      return;
    }
    if (existing.user_id) {
