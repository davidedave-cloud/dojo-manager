...
export default function AthletePortal({ session, supabase }) {
  ...
  async function handleAlreadyMember() {
    ...
    // Prova a creare l'account auth — se l'email non esiste nel DB lo scopriremo dopo
    const { data: authData, error: authError } = await supabase.auth.signUp({ email: email.trim(), password });
    if (authError) {
      // "User already registered" significa che ha già un account -> usa login normale
      if (authError.message.includes("already registered") || authError.message.includes("already been registered")) {
        setAlreadyMember(p => ({...p, loading: false, error: "Hai già un account. Usa il pulsante Accedi con la stessa email."}));
      } else {
        setAlreadyMember(p => ({...p, loading: false, error: "Errore: " + authError.message}));
      }
      return;
    }
    // Cerca l'atleta nel DB con questa email (case-insensitive)
    const { data: existingList } = await supabase.from("athletes").select("id, first_name, last_name, email, user_id").ilike("email", email.trim());
    const existing = existingList && existingList.length > 0 ? existingList[0] : null;
    if (!existing) {
      // Account auth creato ma email non nel DB atleti — elimina account e mostra errore
      setAlreadyMember(p => ({...p, loading: false, error: "Email non trovata nel sistema. Assicurati di usare la stessa email che hai dato in segreteria, oppure contattaci."}));
      return;
    }
    // Collega l'account auth all'atleta
