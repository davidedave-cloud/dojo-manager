...
function AddMemberModal({ onClose, onSave, parentAthlete }) {
  ...
  function validate() {
    if (step === 0 && (!m.firstName || !m.lastName || !m.birthDate || !m.birthPlace || !m.fiscalCode)) return "Compila tutti i campi obbligatori (*)";
    if (step === 1 && (!m.course || !m.location || !m.discipline)) return "Seleziona corso, sede e disciplina";
      ...
      <div style={{ background: "linear-gradient(135deg,#131008,#0d0c07)", border: "1px solid #2a2010", borderRadius: 20, padding: "36px 32px", width: 500, maxWidth: "100%", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.8)" }}>
        ...
        {step === 0 && (
          <div>
            ...
            <div style={{ marginTop: 12 }}><label style={labelStyle}>Luogo di nascita *</label><input style={inputStyle} value={m.birthPlace} onChange={e => setF("birthPlace", e.target.value)} placeholder="Ferrara" /></div>
        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ marginBottom: 14 }}><label style={labelStyle}>Disciplina *</label><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>{DISCIPLINES.map(d => <button key={d} onClick={() => setF("discipline", d)} style={{ background: m.discipline === d ? "linear-gradient(135deg,#b8860b,#daa520)" : "#0d0c07", color: m.discipline === d ? "#0a0905" : "#8a7a6a", border: `1px solid ${m.discipline === d ? "#daa520" : "#2a2010"}`, borderRadius: 8, padding: "10px 6px", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>{d}</button>)}</div></div>
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ marginTop: 16, padding: "12px 16px", background: "#0d0c07", border: "1px solid #1a1408", borderRadius: 8, fontSize: 12, color: "#5a5040", lineHeight: 1.8 }}>
              ...
              <div>🥋 {m.course} · {m.location} · {m.discipline} · Cintura {m.belt}</div>
            </div>
          </div>
        )}
...
export default function AthletePortal({ session, supabase }) {
  ...
  async function handleAddMember(memberData) {
    const { error } = await supabase.from("athletes").insert({
      ...
      fiscal_code: memberData.fiscalCode,
      ...
      belt: memberData.belt, course: memberData.course, location: memberData.location, discipline: memberData.discipline,
                ...
                <div key={m.id} style={{ background: "#131008", border: "1px solid #2a2010", borderRadius: 14, padding: 18, marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: "#5a5040", marginTop: 3 }}>{m.course} · {m.location} · {m.discipline}</div>
          ...
          {activeTab === "pagamenti" && (
            <div>
              ...
              {payments.length === 0 ? <div style={{ textAlign: "center", padding: 60, color: "#5a5040" }}>Nessun pagamento registrato.</div>
                : payments.map(p => <div key={p.id} style={{ background: "#131008", border: "1px solid #2a2010", borderRadius: 12, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}><div><div style={{ fontSize: 14, fontWeight: 600, color: "#e8e0d0" }}>{p.period_month}/{p.period_year}</div><div style={{ fontSize: 11, color: "#5a5040", marginTop: 2 }}>{p.receipt_number}</div></div><span style={{ fontSize: 16, fontWeight: 700, color: "#daa520" }}>€{p.amount}</span></div>)}
            </div>
          )}
          {activeTab === "esami" && (
            <div>
